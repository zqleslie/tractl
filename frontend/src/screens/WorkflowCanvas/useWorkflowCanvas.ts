...
function resolveStepStartTimes(steps: WorkflowStep[]): Map<string, number> {
  ...
  for (const step of steps) {
    if (step.dependsOn.length === 0) {
      computed.set(step.id, 0)
    } else {
      const depEndTimes = step.dependsOn.map((depId) => {
        const depStart = computed.get(depId) ?? 0
        const depStep = steps.find((s) => s.id === depId)
        return depStart + (depStep?.result?.durationMs ?? 0)
      })
      computed.set(step.id, Math.max(...depEndTimes))
    }
  }
...
function deriveRunSummary(
  ...
  const totalDurationMs = engineDurationMs ?? ran.reduce((sum, step) => sum + (step.result?.durationMs ?? 0), 0)
...
export function buildTopologySummary(steps: WorkflowStep[]): string {
...
export function useWorkflowCanvas() {
  ...
  useEffect(() => {
    if (activeWorkflowId === null) {
      ...
      useWorkflowWorkspaceStore.getState().upsertWorkflow(newWorkflow, {
        sourceName: newWorkflow.name,
        sourceFormat: 'yaml',
      })
      useUiStore.getState().openWorkflow(newWorkflow.id)
      loadWorkflowIntoCanvas(newWorkflow)
      return
    }

    const canvasWorkflow = useWorkflowCanvasStore.getState().workflow
    if (canvasWorkflow?.id === activeWorkflowId) return
